# Core of shashin's home: identity, packages, HM self-management.
# Sibling files in this directory merge into the same
...
{ inputs, ... }:
{
  ...
  flake.modules.homeManager.shashin =
    ...
    let
      ...
      # (services.gnome.gnome-keyring.enable in the nostromo noctalia module;
      # no GNOME desktop required — it implements the same Secret Service
      # D-Bus API). Confirmed via ~/.config/Signal/config.json: without the
      # flag, Signal falls back to storing its SQLCipher key in plaintext
      # ("key"); with it, the key is wrapped by the keyring ("encryptedKey").
